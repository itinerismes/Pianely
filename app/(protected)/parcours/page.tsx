...
export default function ParcoursPage() {
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 pt-24 pb-16 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 relative z-10">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-bold text-3xl tracking-tight leading-tight text-white mb-1.5" style={{ letterSpacing: '-0.02em' }}>
            Ton parcours d&apos;apprentissage
          </h1>
          <p className="font-normal text-base text-slate-400 mt-1.5">
        ...
        <div className="space-y-4">
          {levels.map((level) => {
            ...
            return (
              <div
                key={level.id}
                className={`bg-slate-800/60 backdrop-blur-md rounded-2xl border border-slate-700/50 p-6 shadow-xl shadow-black/10 transition-all duration-300 border-l-4 ${level.borderColor} ${
                  level.unlocked
                    ? `hover:bg-slate-800/80 hover:border-slate-600/60 ${level.hoverShadow} hover:shadow-xl`
                    : 'opacity-60'
                }`}
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-start gap-4 flex-1">
                    {/* Icon */}
                    <div
                      className={`w-14 h-14 rounded-xl bg-gradient-to-br ${level.color} flex items-center justify-center flex-shrink-0 shadow-lg ${level.shadowColor}`}
                    >
                      {level.unlocked ? (
                    ...
                    <div className="flex-1">
                      ...
                      <p className="font-normal text-sm text-slate-400 leading-relaxed mb-4">
                        {level.description}
                      </p>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-baseline">
                          <span className="font-medium text-xs text-slate-500 tabular-nums">
                            {level.completedLessons} / {level.totalLessons} leçons complétées
                          </span>
                          <span className="font-bold text-sm tabular-nums text-slate-300">
                            {progressPercent}%
                          </span>
                        </div>

                        {/* Progress bar */}
                        <div className="bg-slate-700/30 h-2 rounded-full w-full overflow-hidden">
                          <div
                  ...
                  <div className="flex-shrink-0">
                    {level.unlocked ? (
                      <Link
                        href={level.href}
                        className={`inline-flex items-center gap-2 px-6 py-2.5 ${level.buttonBg} text-white font-semibold text-sm rounded-xl shadow-lg ${level.buttonShadow} hover:shadow-xl hover:scale-105 transition-all duration-200 border border-white/20`}
                      >
                        {level.completedLessons === 0 ? 'Commencer' : 'Continuer'}
                        <ArrowRight className="w-4 h-4" />
                      </Link>
                    ) : (
                      <div className="bg-slate-700/30 text-slate-500 font-medium text-xs px-3 py-1.5 rounded-lg border border-slate-600/30 inline-flex items-center gap-2">
                ...
                {!level.unlocked && (
                  <div className="mt-4 pt-4 border-t border-slate-700/50">
                    <div className="bg-slate-800/40 text-slate-500 font-medium text-xs px-4 py-2 italic border-l-2 border-slate-700 flex items-center gap-2">
